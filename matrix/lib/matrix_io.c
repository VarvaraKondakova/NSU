...
int freeMatrix(Matrix_NxM *matrix)
{
    if (matrix == NULL)
    {
        perror("The matrix is not initialized!\n");
        return 0;
    }
...
int searchingForElementIndexDynamicMatrix(const Matrix_NxM *matrix, int row, int column)
{
    ...
    {
        perror("Incorrect element indexes in the matrix!\n");
        return 0;
    }
...
int readingMatrixFromFile(Matrix_NxM *matrix, const char *pathToFile)
{
    ...
    {
        perror("The matrix or the file path is not initialized!\n");
        return 0;
    }
    ...
    {
        perror("Incorrect file format - the dimensions of the matrix could not be read!\n");
        fclose(file);
        return 0;
    }
